    ...
    Crafty.c("Board", {
        ...
        _mouseDown: function(obj) {
            ...
            window._miniLoop = setInterval(function() {            
                var selectedColor = board[aPos.x][aPos.y]._color;
                console.log("Current: "+aPos.x+","+aPos.y);
                console.log("Mouse Over: "+window._mouseOverBox.x+","+window._mouseOverBox.y);

                // Constantly update the selected box to be above current mouse.
                board[aPos.x][aPos.y]._color = board[window._mouseOverBox.x][window._mouseOverBox.y]._color;
                board[window._mouseOverBox.x][window._mouseOverBox.y]._color = selectedColor;
                aPos = window._mouseOverBox;
        ...
        _mouseOver: function(obj) {
            var objPos = this._translateToArrayPos(obj.x, obj.y);
            window._mouseOverBox = objPos;
        },
        // Mouse up handler
        ...
        _flagConnectedBoxes: function(aPos, color) {
            ...
            for (var i = 0; i < BOARD_COLS; i++) {
                for (var j = 0; j < BOARD_ROWS; j++) {
                    if(board[i] && board[i+2] && sameColor(board[i+1][j], board[i][j]) 
                        && sameColor(board[i+1][j], board[i+2][j])) {
                        board[i+1][j]._flagged = true;
                        board[i][j]._flagged = true;
                        board[i+2][j]._flagged = true;
                        console.log(j,i);
                        console.log(board[i][j]._color);
                    }
                    if(board[i][j] && board[i][j+2] && sameColor(board[i][j+1], board[i][j]) 
                        && sameColor(board[i][j+1], board[i][j+2])) {
                        board[i][j+1]._flagged = true;
                        board[i][j]._flagged = true;
                        board[i][j+2]._flagged = true;
                        console.log(j,i);
                        console.log(board[i][j]._color);
                    }
                };
